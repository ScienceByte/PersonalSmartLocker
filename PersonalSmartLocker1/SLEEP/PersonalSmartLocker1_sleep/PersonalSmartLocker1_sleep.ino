...
void setup() {
  ...
  pinMode(A5, INPUT); //used for keypad read
  ...
  //PCICR |= (1 << PCIE1);    // Enable Pin Change Interrupt group 1 (Port C, PCINT8-14)
  PCMSK1 |= (1 << PCINT13); // Enable the specific interrupt for PCINT13 (A5)
...
ISR(PCINT1_vect){ 
  ...
  PCICR &= ~(1 << PCIE1); // Disable Pin Change Interrupt group 1 (Port C)
  justWoke = true;
}

void goToSleep(){
  //EIMSK |= (1<<INT0); // external interrupt enable on INT0
  PCICR |= (1 << PCIE1); // Enable Pin Change Interrupt group 1 (Port C)
...
void loop() {
  ...
  if (millis() - previousSerialMillis >= serialInterval) {
    previousSerialMillis = millis(); // Reset the print timer
     anval = analogRead(A0);
     //Serial.println(anval); //this is the servo shunt resistor reading
    obstructionReturn();
  }
}